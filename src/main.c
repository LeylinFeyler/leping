...
ping_stats stats = {
    .transmitted = 0, .received = 0, .rtt_min = 1e9, .rtt_max = 0, .rtt_sum = 0, .rtt_sum_sq = 0};
...
int main(int argc, char **argv) {
    ...
    printf("LEPING %s (%s)\n", reverse_dns((struct sockaddr *)&addr, addr_len), ip);
    ...
    while (running) {
        if (opts.count != -1 && seq >= opts.count) {
            break;
        }

        struct timeval send_time;

        if (send_ping(sockfd, (struct sockaddr *)&addr, addr_len, seq, opts.packet_size,
                      &send_time) == 0) {
